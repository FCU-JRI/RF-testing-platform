...
int LoRaClass::parsePacket(int size)
{
  ...
  if (size > 0) {
    ...
    if (readRegister(REG_PAYLOAD_LENGTH) != (size & 0xff)) {
  ...
  if ((irqFlags & IRQ_RX_DONE_MASK) && (irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) == 0) {
    ...
    // put in standby mode
    idle();
  } else if ((irqFlags & IRQ_RX_DONE_MASK) && (irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) != 0) {
    // CRC Error detected
    writeRegister(REG_FIFO_ADDR_PTR, 0);
    writeRegister(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_RX_SINGLE);
    packetLength = -1; // Indicate CRC error
...
{
  _implicitHeaderMode = 0;
  uint8_t val = readRegister(REG_MODEM_CONFIG_1);
  if (val & 0x01) {
    writeRegister(REG_MODEM_CONFIG_1, val & 0xfe);
  }
}
...
void LoRaClass::implicitHeaderMode()
{
  ...
  uint8_t val = readRegister(REG_MODEM_CONFIG_1);
  if (!(val & 0x01)) {
    writeRegister(REG_MODEM_CONFIG_1, val | 0x01);
  }
}
